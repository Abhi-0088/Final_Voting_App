...
contract VotingSystem {
    ...
    function approveClubCreation(uint _id) public {
        require(msg.sender == superuser, "You are not authorized to approve club creation.");
        for (uint i = 0; i < club_creation_requests.length; i++) {
            if (club_creation_requests[i].id == _id) {
                if (club_name_to_club[club_creation_requests[i].club_name].exists) {
                    ...
                    // Update the club_name_to_club mapping first
                    club_name_to_club[new_club.name] = new_club;
                    

                    // Push the new club to club_list
                    club_list.push(club_name_to_club[new_club.name]);

                    // Now add the creator as both member and admin
                    addMemberAdminToClub(club_creation_requests[i].creator_id, new_club.id);
    ...
    function getMembers(string calldata _club_name) public view returns (MemberData[] memory) {
        ...
        Club storage club = club_name_to_club[_club_name];
        MemberData[] memory result = new MemberData[](club.members.length);

        for (uint i = 0; i < club.members.length; i++) {
            Student memory temp = getStudentDetailsById(club.members[i]);
            result[i] = MemberData({
                id: temp.id,
                name: temp.name,
    ...
    function getWaiting(string calldata _club_name) public view returns (MemberData[] memory) {
        require(club_name_to_club[_club_name].exists, "Club does not exist!");
        MemberData[] memory result = new MemberData[](club_name_to_club[_club_name].waitlist.length);
        Student memory temp;
    ...
    function applyForClubMembership(string calldata _club_name) public {
        ...
        bool found;
        for (uint i = 0; i < target.waitlist.length; i++) {
        ...
        for (uint i = 0; i < roll_no.length; i++) {
            ...
            for (j = 0; j < club_name_to_club[_club_name].waitlist.length; j++) {
                if (club_name_to_club[_club_name].waitlist[j] == temp.id) {
                    addMemberToClub(temp.id, club_name_to_club[_club_name].id);
   ...
   function getMyClubs() public view returns (ClubData[] memory) {
    ...
    function createElection(
        ...
        election_list.push(new_election.id);
        return electionId;
    }
    ...
    function getElections() public view returns (ElectionDetails[] memory) {
        ElectionDetails[] memory result = new ElectionDetails[](election_list.length);
        for (uint i = 0; i < election_list.length; i++) {
            result[i].id = election_list[i];
            result[i].name = elections[election_list[i]].name;
            result[i].desc = elections[election_list[i]].desc;
            result[i].startTime = elections[election_list[i]].startTime;
            result[i].endTime = elections[election_list[i]].endTime;
            result[i].candidates = new uint[](elections[election_list[i]].candidates.length);
            for (uint j = 0; j < elections[election_list[i]].candidates.length; j++) {
                result[i].candidates[j] = elections[election_list[i]].candidates[j];
            }
        }
        return result;
    }
